...
export default async function ClientesPage({
  ...
  /* A carteira é da rede inteira — nome, telemóvel e histórico de toda a gente.
     Quem atende vê a cliente do próprio horário pela ficha do atendimento. */
  ...
  return (
    <PainelShell
      ...
      activa="clientes"
      titulo="Clientes"
      descricao="Ficha, preferências e histórico de visitas"
      /* A carteira é da rede: recortá-la pela casa activa esconderia a cliente
         que costuma ir à outra loja, que é justamente quem a recepção procura
         quando ela liga. */
      semCasa
      acao={
        <Link
          href={href('/painel/clientes/importar')}
          className="rounded-plate text-muted py-1 text-sm whitespace-nowrap transition-colors hover:text-(--text-strong)"
        >
          importar CSV →
        </Link>
      }
              ...
              <ul className="border-t border-(--border-strong)">
                {linhas.map((client) => (
                  <li key={client.clientId}>
                    <Link
                      href={href(`/painel/clientes/${client.clientId}`)}
