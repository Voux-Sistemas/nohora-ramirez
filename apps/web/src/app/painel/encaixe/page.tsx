...
export default async function EncaixePage({
  ...
  /* A casa vem da barra do painel: encaixa-se na loja em que se está a olhar,
     e não na que estava escrita no endereço de onde se veio. */
  const { acesso, unidade: unit } = await contextoDoPainel()
  ...
  const base = `/painel/encaixe?d=${date}`
  ...
  if (query.t) {
    ...
    return (
      ...
      >
        ...
        <form action={`/painel/encaixe`} className="mb-5 flex gap-2">
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 flex gap-2">
        <Link
          href={href(`/painel/encaixe?d=${addDaysInZone(date, -1)}&s=${serviceIds.join(',')}${staffId ? `&p=${staffId}` : ''}`)}
...
function Shell({
  ...
  return (
    <main className="mx-auto w-full max-w-2xl px-5 py-8 pb-28 sm:px-8">
      <Link href={href(back ?? `/painel?d=${date}`)} className="text-muted text-sm hover:underline">
        ← voltar
      </Link>
      {/*
