...
export default async function ProfissionalFormPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AdminShell acesso={acesso} active="/painel/gestao/equipe" title={isNew ? 'Novo profissional' : staff.name}>
      <Link href={backTo('/painel/gestao/equipe')} className="text-muted mb-4 inline-block text-sm hover:underline">
        ← equipe
      </Link>
