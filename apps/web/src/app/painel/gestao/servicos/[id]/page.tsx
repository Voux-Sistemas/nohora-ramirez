...
export default async function ServicoFormPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AdminShell acesso={acesso} active="/painel/gestao/servicos" title={isNew ? 'Novo serviço' : service.name}>
      <Link href={backTo('/painel/gestao/servicos')} className="text-muted mb-4 inline-block text-sm hover:underline">
        ← serviços
      </Link>
