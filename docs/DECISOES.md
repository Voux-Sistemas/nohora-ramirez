...
**Consequências:**
- **Duas URLs, não uma — e as duas no pooler de sessão (5432).** `DATABASE_URL` como o papel `app_web`, que só faz DML, para o site; `DIRECT_URL` como o papel dono, para migration, constraints e seed. Diferem no papel, não na porta. Nunca o pooler de transação (6543) para o site, por mais que a Supabase o recomende para aplicações web: ele engole consultas encavalitadas e o `/agendar` deixa de responder — medido a 2026-08-18, ver o comentário em `packages/db/src/index.ts`. Nunca o endereço "Direct connection": é IPv6-only e a saída do Railway é IPv4.
...
- **O rollback tinha prazo, e o prazo passou.** O Postgres do Railway ficou de pé e intocado enquanto a migração assentava, e foi apagado a seguir. Voltar atrás já não é repor duas variáveis: é restaurar um backup, e a escrita feita no Supabase desde o corte é a agenda real do salão. A rede que sobra é a de `ops/backup`, provada todas as noites.
